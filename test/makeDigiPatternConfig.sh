...
#if [ true ]
then
	echo "need to include a name for the pattern"
	exit $EXIT_BADARGS
fi
...
cat <<EOF > 'rctEcalHcalFromDigis_cfg.py'
...
process.rctSaveInput = cms.EDFilter("L1RCTSaveInput",
    ...
    digiFile = cms.untracked.bool(True)
)
...
process.p = cms.Path(process.rctSaveInput*process.L1RCTPatternTestAnalyzer * process.ecalSimRawData * process.htr_xml )#*process.filter
...
process.ecalSimRawData.outputBaseName = 'ecalData/${patternName}/ecal'

EOF


cat <<EOF > 'rctFromDigis_cfg.py'
